...
Example usage with nanotron:
===================
CUDA_DEVICE_MAX_CONNECTIONS=1 python -m torch.distributed.launch --use-env \
      --nproc_per_node 4 \
      "/capstor/scratch/cscs/ahgele/lighteval/run_evals_nanotron.py" \
      --checkpoint-config-path="/capstor/scratch/cscs/ahgele/nanotron/checkpoints-1B-fineweb-100BT-rt0.5M/55000/config.yaml"\
      --lighteval-override="/capstor/scratch/cscs/ahgele/pretrain/configs/smollm_eval.yaml"
===================
...
COMMON_SENSE_REASONING_STRING = [
    (t, f"custom|{t.name}|0|1") for t in COMMON_SENSE_REASONING_TASKS
]
...
TASKS_TABLE = [task for task in _TASKS]
